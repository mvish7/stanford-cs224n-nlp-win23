...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    ### ### we need to implement following from lecture1.
    ### -1/T sum over t=1 ... T log prob(outside word given center word)

    ### ### Step1: implement log prob(outside word given center word) -- nothing but softmax
    pred = softmax(np.dot(outsideVectors, centerWordVec))

    ### ### Step2: include log
    loss = -np.log(pred[outsideWordIdx])

    ### ### Step3: refer to partial derivative calculations in lecture 1 for gradient calculations
    # prepare GT by assigning probability of 1 to the word @ outsideWordIdx
    gt = np.zeros(pred.shape)
    gt[outsideWordIdx] = 1
    # the dot product below calculates the summation of error in each element in pred and gt
    gradCenterVec = np.dot(outsideVectors.T, pred - gt)
    gradOutsideVecs = np.outer(pred - gt, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    ### # step 1: implement loss
    true_pair_loss = -np.log(
        sigmoid(np.dot(centerWordVec, outsideVectors[outsideWordIdx].T))
    )
    neg_sampling_loss = -np.sum(
        np.log(sigmoid(np.dot(outsideVectors[negSampleWordIndices].T, centerWordVec)))
    )
    loss = true_pair_loss + neg_sampling_loss

    ### # step 2: implement centerWordVec grad -- refer to partial derivative calculations in Assignment_2

    gradCenterVec_lhs = (
        sigmoid(np.dot(outsideVectors[outsideWordIdx].T, centerWordVec))
        * outsideVectors[outsideWordIdx]
    )
    gradCenterVec_rhs = (
        -(1 - sigmoid(np.dot(-outsideVectors[negSampleWordIndices].T, centerWordVec)))
        * outsideVectors[negSampleWordIndices]
    )

    gradCenterVec = 1 - gradCenterVec_lhs + gradCenterVec_rhs

    ### step 3: calculate grad wrt. window word vec -- partial derivative calculations in Assignment_2
    # in addition to partial derivative calculations, including the case when an outside word is negatively sampled
    # more than once.
    ...
    # finding repeated negative sampled words
    unique, ind, cnt = np.unique(
        negSampleWordIndices, return_index=True, return_count=True
    )
    unique_sampled = sigmoid(
        -np.dot(outsideVectors[negSampleWordIndices], centerWordVec)
    )[indices]
    gradOutsideVecs[unique, :] = ((1 - unique_sampled) * cnt)[:, None] * centerWordVec[
        None, :
    ]
...
def skipgram(
    ...
    cword_id = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[cword_id]

    for word in outsideWords:
        outsideWordIdx = word2Ind[word]
        skip_gram_loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(
            centerWordVec, outsideWordIdx, outsideVectors, dataset
        )
        loss += skip_gram_loss
        gradCenterVecs[cword_id] += gradCenterVec
